...
function Input({ value, onChange, placeholder, type = "text", disabled = false }) {
    return (
        <input
            ...
            disabled={disabled}
            className="w-full px-3 py-2.5 rounded-xl text-sm font-medium text-gray-800 outline-none transition-all disabled:opacity-50 disabled:cursor-not-allowed"
            style={{ background: "rgba(78,174,229,0.05)", border: "1.5px solid rgba(78,174,229,0.2)" }}
            onFocus={e => { if (!disabled) e.target.style.borderColor = "#4EAEE5"; }}
...
// ─── Section card with edit/save header ───────────────────────────────────────
    ...
    return (
        <div
            ...
            style={{
                ...
                border: editMode ? `1.5px solid ${color}40` : "1.5px solid rgba(78,174,229,0.14)",
                backdropFilter: "blur(10px)",
                boxShadow: editMode
                    ? `0 8px 32px ${color}18`
                    : "0 4px 20px rgba(78,174,229,0.06)",
        ...
        >
            <div className="flex items-center justify-between mb-5">
                ...
                <button
                    onClick={onToggle}
                    disabled={saving}
                    className="flex items-center gap-2 px-4 py-2 rounded-xl text-xs font-bold transition-all duration-150 active:scale-95"
                    style={editMode
                        ? { background: `linear-gradient(135deg,${color},${color}bb)`, color: "#fff", boxShadow: `0 4px 14px ${color}40` }
...
function EduCard({ item, index, edit, onChange, onRemove }) {
    return (
        <div className="rounded-2xl p-4 mb-3 relative" style={{ background: "rgba(155,89,182,0.05)", border: "1.5px solid rgba(155,89,182,0.14)" }}>
            {edit && (
                <button onClick={() => onRemove(index)} className="absolute top-3 right-3 w-7 h-7 rounded-lg flex items-center justify-center text-red-400 hover:bg-red-50 transition-colors">
                    <FiTrash2 size={13} />
                </button>
            )}
            {edit ? (
...
function Toast({ msg, type, onDone }) {
    ...
    return (
        <div
            className="fixed bottom-6 left-1/2 -translate-x-1/2 z-[9999] flex items-center gap-2.5 px-5 py-3 rounded-2xl text-sm font-bold pointer-events-none"
            style={{
                background: ok ? "#22c55e" : "#ef4444",
                color: "#fff",
                animation: "toastUp 0.3s ease both",
                boxShadow: ok ? "0 8px 30px rgba(34,197,94,0.4)" : "0 8px 30px rgba(239,68,68,0.4)",
                whiteSpace: "nowrap",
            }}
        >
            ...
            {ok ? <FiCheck size={15} /> : <FiAlertCircle size={15} />}
            {msg}
        </div>
    );
}
...
// ─── Profile hero ─────────────────────────────────────────────────────────────
function ProfileHero({ member, from, to, onPhotoClick }) {
    ...
    return (
        ...
        >
            {/* Decorative circle */}
            <div className="absolute -right-12 -top-12 w-52 h-52 rounded-full pointer-events-none opacity-10"
            ...
            <div className="flex items-start gap-5 relative">
                ...
                <div className="relative group flex-shrink-0 cursor-pointer" onClick={onPhotoClick}>
                    <div
                        className="flex items-center justify-center rounded-2xl font-black text-white select-none overflow-hidden"
                        style={{ width: 80, height: 80, background: `linear-gradient(135deg,${from},${to})`, fontSize: 28, boxShadow: `0 8px 28px ${from}50` }}
                    >
                        {member.photo
                ...
                <div className="flex-1 min-w-0">
                    <h1 className="text-2xl sm:text-3xl font-black text-gray-900 leading-tight truncate">{member.name}</h1>
                    <p className="text-sm font-mono text-gray-400 mt-0.5 mb-3">{member.memberId}</p>
                    <div className="flex flex-wrap gap-2">
                        ...
                        {member.bloodGroup && member.bloodGroup !== "Unknown" && (
                            <span className="inline-flex items-center gap-1.5 px-3 py-1 rounded-full text-xs font-bold"
                                ...
                                <FiHeart size={10} /> {member.bloodGroup}
                            </span>
                        )}
                        <span className="inline-flex items-center gap-1.5 px-3 py-1 rounded-full text-xs font-bold"
                        ...
                        {member.place && (
                            <span className="inline-flex items-center gap-1.5 px-3 py-1 rounded-full text-xs font-bold"
                                style={{ background: "rgba(67,184,156,0.1)", color: "#085041", border: "0.5px solid rgba(67,184,156,0.3)" }}>
                                <FiMapPin size={9} /> {member.place}
                            </span>
                        )}
                        <span className="inline-flex items-center gap-1.5 px-3 py-1 rounded-full text-xs font-bold"
                            style={{ background: "rgba(78,174,229,0.1)", color: "#185FA5", border: "0.5px solid rgba(78,174,229,0.25)" }}>
                            <FiShield size={9} /> Since {fmtDate(member.createdAt)}
                        </span>
                    </div>
                </div>
            </div>
...
export default function ProfilePage() {
    ...
    useEffect(() => {
        ...
        (async () => {
            try {
                ...
                setMember(m);
                // ── Map ALL schema fields into draft ──────────────────────────
                setDraft({
                    // Core — schema required fields (editable but validated on server)
                    ...
                    // Spouse — only used when married (schema: `spouse` field)
                    spouse:         m.spouse         || "",
                    // Contact
                    phone:          m.phone          || "",
                    email:          m.email          || "",
                    ...
                    // About
                    bio:            m.bio            || "",
                    // Career (not in schema yet — sent as extra fields)
    ...
    const setArr = (key, i, field, val) =>
        setDraft(d => { const a = [...(d[key] || [])]; a[i] = { ...a[i], [field]: val }; return { ...d, [key]: a }; });
    const addArr = (key, blank) =>
        setDraft(d => ({ ...d, [key]: [...(d[key] || []), blank] }));
    const rmArr  = (key, i) =>
        setDraft(d => { const a = [...(d[key] || [])]; a.splice(i, 1); return { ...d, [key]: a }; });

    // ── Save section ───────────────────────────────────────────────────────────
    const toggleEdit = async (section) => {
        if (!editMode[section]) {
            // Enter edit mode — close all others first
        ...
        try {
            // Build the subset of `draft` relevant to this section
            const payload = buildPayload(section, draft);
            const res  = await updateMember(member._id, payload);
            const updated = res.data ?? res;
            // Merge updated fields back into member so view reflects save
            setMember(m => ({ ...m, ...updated }));
            setDraft(d  => ({ ...d,  ...updated, dob: updated.dob ? new Date(updated.dob).toISOString().split("T")[0] : d.dob }));
    ...
    return (
        <div className="min-h-screen" style={{ background: "linear-gradient(145deg,#f0f4ff 0%,#faf5ff 50%,#f0fff8 100%)" }}>
            ...
            <div className="relative z-10 max-w-3xl mx-auto px-4 pt-24 pb-20">
                ...
                {/* Hero */}
                <ProfileHero member={member} from={from} to={to} onPhotoClick={() => {/* TODO: photo upload */}} />

                <div className="space-y-5">

                    {/* ── 1. Personal Details ── */}
                    <SectionCard
                        icon={FiUser} title="Personal Details" color="#9B59B6"
                        editMode={editMode.personal} saving={saving.personal}
                        onToggle={() => toggleEdit("personal")}
                        hint="DOB, gender, blood group, parents"
                    >
                        <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
                            ...
                            <Field label="Gender" value={draft.gender} edit={editMode.personal}>
                                <SelectInput value={draft.gender} onChange={set("gender")} options={["Male", "Female", "Other"]} placeholder="Select gender" />
                            </Field>
                            <Field label="Blood Group" value={draft.bloodGroup} edit={editMode.personal}>
                                <SelectInput value={draft.bloodGroup} onChange={set("bloodGroup")} options={BLOOD_GROUPS} placeholder="Select blood group" />
                            </Field>
                            <Field label="Marital Status" value={draft.maritalStatus} edit={editMode.personal}>
                                <SelectInput value={draft.maritalStatus} onChange={set("maritalStatus")} options={MARITAL_OPTIONS} placeholder="Select status" />
                            </Field>
                            <Field label="Father's Name" value={draft.father} edit={editMode.personal}>
                                <Input value={draft.father} onChange={set("father")} placeholder="Father's full name" />
                            </Field>
                            <Field label="Mother's Name" value={draft.mother} edit={editMode.personal}>
                                <Input value={draft.mother} onChange={set("mother")} placeholder="Mother's full name" />
                            </Field>
                        </div>

                        {/* Spouse — shown only when Married */}
                        {isMarried && (
                            <>
                                ...
                                {/* Schema stores spouse as a single String field */}
                                <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
                                    <Field label="Spouse Name" value={draft.spouse} edit={editMode.personal} span2>
                                        <Input value={draft.spouse} onChange={set("spouse")} placeholder="Spouse's full name" />
                                    </Field>
                                </div>
                            </>
                        )}
                    ...
                    <SectionCard
                        icon={FiPhone} title="Contact Information" color="#4EAEE5"
                        editMode={editMode.contact} saving={saving.contact}
                        onToggle={() => toggleEdit("contact")}
                        hint="Phone, email and address"
                    >
                        <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
                            <Field label="Phone Number" value={draft.phone} edit={editMode.contact}>
                                <Input value={draft.phone} onChange={set("phone")} placeholder="9XXXXXXXXX" type="tel" />
                            </Field>
                            <Field label="Email Address" value={draft.email} edit={editMode.contact}>
                                <Input value={draft.email} onChange={set("email")} placeholder="you@example.com" type="email" />
                            </Field>
                            <Field label="Place / City" value={draft.place} edit={editMode.contact}>
                                <Input value={draft.place} onChange={set("place")} placeholder="City or Town" />
                            </Field>
                            <Field label="House Name" value={draft.houseName} edit={editMode.contact}>
                    ...
                    {/* ── 3. About Me ── */}
                    <SectionCard
                        icon={FiUsers} title="About Me" color="#43B89C"
                        editMode={editMode.bio} saving={saving.bio}
                        onToggle={() => toggleEdit("bio")}
                        hint="Short bio visible to members"
                    >
                        <Field label="Bio / Short Note" value={draft.bio} edit={editMode.bio}>
                            <Textarea value={draft.bio} onChange={set("bio")} placeholder="Write a short note about yourself…" rows={4} />
                        </Field>
                    </SectionCard>

                    {/* ── 4. Career & Employment ── */}
                    <SectionCard
                        icon={FiBriefcase} title="Career & Employment" color="#F7971E"
                        editMode={editMode.career} saving={saving.career}
                        onToggle={() => toggleEdit("career")}
                        hint="Job, sector and income"
                    >
                        <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
                            <Field label="Employment Type" value={draft.employmentType} edit={editMode.career} span2>
                            ...
                            <Field label="Sector" value={draft.sector} edit={editMode.career}>
                                <SelectInput value={draft.sector} onChange={set("sector")} options={SECTORS} placeholder="Select sector" />
                            </Field>
                            <Field label="Organisation" value={draft.organisation} edit={editMode.career}>
                                <Input value={draft.organisation} onChange={set("organisation")} placeholder="Company / Org" />
                            </Field>
                            <Field label="Job Title" value={draft.jobTitle} edit={editMode.career}>
                                <Input value={draft.jobTitle} onChange={set("jobTitle")} placeholder="Designation" />
                            </Field>
                            ...
                            <Field
                                label="Annual Income (₹)"
                                value={draft.annualIncome ? `₹ ${Number(draft.annualIncome).toLocaleString("en-IN")}` : ""}
                        ...
                        <Field label="Skills" value={draft.skills} edit={editMode.career}>
                    ...
                    {/* ── 5. Work Experience ── */}
                    <SectionCard
                        icon={FiAward} title="Work Experience" color="#E91E8C"
                        editMode={editMode.experience} saving={saving.experience}
                        onToggle={() => toggleEdit("experience")}
                        hint={`${expCount} ${expCount === 1 ? "entry" : "entries"}`}
                    >
                    ...
                    {/* ── 6. Education ── */}
                    <SectionCard
                        icon={FiBook} title="Education" color="#9B59B6"
                        editMode={editMode.education} saving={saving.education}
                        onToggle={() => toggleEdit("education")}
                        hint={`Highest: ${draft.highestQualification || "not set"}`}
                    >
                        <div className="grid grid-cols-1 sm:grid-cols-2 gap-4 mb-5">
                            <Field label="Highest Qualification" value={draft.highestQualification} edit={editMode.education} span2>
                        ...
                        <div className="flex items-center justify-between mb-3">
                            ...
                            {editMode.education && (
                                <button
                                    onClick={() => addArr("educations", { level: "", field: "", institution: "", year: "" })}
                                    className="flex items-center gap-1.5 px-3 py-1.5 rounded-xl text-xs font-bold active:scale-95"
                                    style={{ background: "rgba(155,89,182,0.1)", color: "#534AB7", border: "1px solid rgba(155,89,182,0.2)" }}>
                                    <FiPlus size={11} /> Add
                                </button>
                    ...
                    <div className="rounded-3xl p-6" style={{ background: "rgba(255,255,255,0.6)", border: "1.5px solid rgba(78,174,229,0.1)" }}>
                        <div className="flex items-center gap-3 mb-5">
                            ...
                            <div>
                                ...
                                <p className="text-[11px] text-gray-400">Read-only · Contact admin to update</p>
                            </div>
                        </div>
...
// ─── Build the PATCH payload per section ──────────────────────────────────────
// Only sends the fields belonging to that section — keeps requests minimal
// and avoids accidentally overwriting unrelated data.
function buildPayload(section, draft) {
    switch (section) {
        case "personal":
            return {
                dob:           draft.dob,
                ...
                father:        draft.father,
                mother:        draft.mother,
                // Only include spouse when married; clear it otherwise
        ...
        case "experience":
            return { experiences: draft.experiences };
        case "education":
            return {
